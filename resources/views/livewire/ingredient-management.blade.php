    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($products as $product)
            <div class="border p-4 rounded">
                ...
                <button wire:click="editProduct({{ $product->id }})" class="bg-yellow-500 text-white px-2 py-1 rounded mt-2">Edit</button>
            </div>
        @endforeach
    </div>
    ...
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" id="my-modal">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <div class="flex justify-end">
                <button wire:click="updateProduct" class="bg-blue-500 text-white px-4 py-2 rounded mr-2">Save</button>
                <button wire:click="$set('editingProduct', null)" class="bg-gray-300 px-4 py-2 rounded">Cancel</button>
            </div>
        </div>
    </div>
    @endif
