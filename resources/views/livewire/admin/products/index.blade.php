<div>
    <!-- Header Section -->
    <x-theme.breadcrumb :title="__('Product Management')" :parent="route('admin.products')" :parentName="__('Product Management')">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <x-stats-card title="{{ __('Total Products') }}" value="{{ $this->productAnalytics['total_products'] }}"
            icon="inventory_2" />

        <x-stats-card title="{{ __('Active Products') }}" value="{{ $this->productAnalytics['active_products'] }}"
            icon="check_circle" iconColor="green" />

        <x-stats-card title="{{ __('Low Stock') }}" value="{{ $this->productAnalytics['low_stock_count'] }}"
            icon="warning" iconColor="yellow" />

        <x-stats-card title="{{ __('Featured') }}" value="{{ $this->productAnalytics['featured_count'] }}"
            icon="star" iconColor="blue" />
    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-input id="cost" wire:model="cost" type="number" step="0.01" class="w-full" />
                            @error('cost')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            ...
                            <x-input id="price" wire:model="price" type="number" step="0.01" class="w-full" />
                            @error('price')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-input id="stock" wire:model="stock" type="number" step="0.01" class="w-full" />
                            @error('stock')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <x-button type="button" wire:click="addSizePrice" color="secondary" size="sm">
                                {{ __('Add Size') }}
                            </x-button>
                        </div>
                        ...
                        @foreach ($sizePrices as $index => $sizePrice)
                            <div class="grid grid-cols-2 gap-4 items-center border p-4 rounded-lg">
                                ...
                                <div class="col-span-2 flex justify-end">
                                    <x-button type="button" wire:click="removeSizePrice({{ $index }})"
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <x-button type="button" wire:click="addIngredient" color="secondary" size="sm">
                                {{ __('Add Ingredient') }}
                            </x-button>
                        </div>
                        ...
                        @foreach ($selectedIngredients as $index => $ingredient)
                            <div class="grid grid-cols-3 gap-4 items-center border p-4 rounded-lg">
                                <div>
                                    <x-label for="selectedIngredients.{{ $index }}.id" :value="__('Ingredient')" />
                                ...
                                <div>
                                    ...
                                    <x-input id="selectedIngredients.{{ $index }}.quantity"
                                        wire:model="selectedIngredients.{{ $index }}.quantity" type="number"
                                        step="0.01" class="w-full" />
                                </div>
                                <div>
                                    <x-label for="selectedIngredients.{{ $index }}.unit" :value="__('Unit')" />
                                ...
                                <div class="col-span-3 flex justify-end">
                                    <x-button type="button" wire:click="removeIngredient({{ $index }})"
                                        color="danger" size="sm">
                                        {{ __('Remove') }}
                                    </x-button>
                                </div>
                ...
                <div
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-2 gap-2 items-center pt-4 border-t">
                            ...
                            <x-button wire:click="openAdjustStockModal('{{ $product->id }}')" color="secondary"
                                size="sm" type="button">
                                {{ __('Adjust Stock') }}
                            </x-button>
                            <!-- Price History Modal -->
                            <x-button wire:click="openPriceHistoryModal('{{ $product->id }}')" color="secondary"
                                size="sm" type="button">
                                {{ __('Price History') }}
                            </x-button>
                            <!-- Stock History Modal -->
                            <x-button wire:click="openStockHistoryModal('{{ $product->id }}')" color="secondary"
                                size="sm" type="button">
                                {{ __('Stock History') }}
                            </x-button>
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            <x-input wire:model="adjustmentQuantity" type="number" step="0.01" class="w-full"
    ...
    <x-modal wire:model="showAdjustStock" name="showAdjustStock">
        <x-slot name="title">
        ...
        <div class="space-y-4">
            <div>
                ...
                <x-input id="adjustmentQuantity" type="number" step="0.01" wire:model="adjustmentQuantity"
                    class="w-full" />
            </div>

            <div>
                <x-label for="adjustmentReason" :value="__('Reason')" />
                <x-input id="adjustmentReason" type="text" wire:model="adjustmentReason" class="w-full" />
            </div>
        </div>

        <div class="mt-6 flex justify-end space-x-3">
            <x-button @click="$wire.set('showAdjustStock', false)" color="secondary">
                {{ __('Cancel') }}
            </x-button>
            <x-button wire:click="adjustStock('{{ $selectedProductId }}')" color="primary">
                {{ __('Save') }}
            </x-button>
        </div>
    ...
    <x-modal wire:model="showPriceHistory" name="showPriceHistory">
        ...
        <div class="mt-6 flex justify-end">
            <x-button @click="$wire.set('showPriceHistory', false)" color="secondary">
                {{ __('Close') }}
            </x-button>
        </div>
    ...
    <x-modal wire:model="showStockHistory" name="showStockHistory">
        ...
        <div class="mt-6 flex justify-end">
            <x-button @click="$wire.set('showStockHistory', false)" color="secondary">
                {{ __('Close') }}
            </x-button>
        </div>
