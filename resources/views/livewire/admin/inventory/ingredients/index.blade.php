<div>
    <div x-data="{
        ...
        <div class="bg-gray-50  rounded-lg shadow-lg py-6 px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-center mb-2">
                ...
                <div class="flex flex-row items-center gap-x-2">
                    <x-button wire:click="$set('showExpenseModal', true)" color="primary" size="xs" type="button">
                        <span class="material-icons text-xl">add</span>
                        <span>{{ __('Add Ingredient') }}</span>
                    </x-button>
                    @if ($selectAll && count($selectedIngredients) > 0)
                        <x-button wire:click="$set('showBulkActions', true)" size="xs" type="button" color="info">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-right text-sm font-medium">
                                    <div class="flex justify-end items-center space-x-2">
                                        ...
                                        <x-button wire:click="openOptimizationModal('{{ $ingredient->id }}')"
                                            color="warning" size="sm" title="Cost Optimization">
                                            <span class="material-icons">trending_up</span>
                                        </x-button>
                                        <x-button wire:click="openSeasonalModal('{{ $ingredient->id }}')"
                                            color="info" size="sm" title="Seasonal Management">
                                            <span class="material-icons">event</span>
                                        </x-button>
                                        <x-button wire:click="openWastageModal('{{ $ingredient->id }}')"
                                            color="danger" size="sm" title="Wastage Metrics">
        ...
        <!-- Stock History Modal -->
        <x-modal wire:model="showStockHistory" name="showStockHistory">
            <div class="p-6">
                <div class="flex justify-between items-center mb-4">
        ...
        <!-- Price History Modal -->
        <x-modal wire:model="showPriceHistory" name="showPriceHistory">
            <x-slot name="title">
        ...
        <x-modal wire:model="showOptimizationModal" name="showOptimizationModal">
            ...
            @if ($selectedIngredient)
                <div class="mb-4 p-4 bg-yellow-50 rounded-lg">
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="font-medium text-yellow-800">{{ $selectedIngredient->name }}</p>
                            <p class="text-sm text-yellow-600">Current Cost:
                                {{ number_format($selectedIngredient->cost, 2) }} DH</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Average Cost (30 days):</span>
                            <span
                                class="font-bold">{{ number_format($selectedIngredient->prices()->where('created_at', '>=', now()->subDays(30))->avg('cost'), 2) }}
                                DH</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Lowest Cost:</span>
                            <span
                                class="font-bold text-green-600">{{ number_format($selectedIngredient->prices()->min('cost'), 2) }}
                                DH</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Highest Cost:</span>
                            <span
                                class="font-bold text-red-600">{{ number_format($selectedIngredient->prices()->max('cost'), 2) }}
                                DH</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    <h4 class="font-medium text-gray-900 mb-2">Cost History</h4>
                    <div class="space-y-2">
                        @foreach ($selectedIngredient->prices()->latest()->take(5)->get() as $price)
                            <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                                <div>
                                    <span class="text-sm text-gray-600">{{ $price->created_at }}</span>
                                </div>
                                <div class="text-sm">
                                    <span class="font-medium">{{ number_format($price->cost, 2) }} DH</span>
                                    @if ($price->cost_change_percentage != 0)
                                        <span
                                            class="{{ $price->cost_change_percentage > 0 ? 'text-red-600' : 'text-green-600' }}">
                                            ({{ $price->cost_change_percentage > 0 ? '+' : '' }}{{ number_format($price->cost_change_percentage, 1) }}%)
                                        </span>
                                    @endif
                                </div>
                            </div>
        ...
        <!-- Seasonal Management Modal -->
        <x-modal wire:model="showSeasonalModal" name="showSeasonalModal">
            <x-slot name="title">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Seasonal Management</h3>
            </x-slot>
            @if ($selectedIngredient)
                <div class="mb-6 bg-blue-50 rounded-lg p-4">
                    <div class="flex justify-between items-center">
                        <h4 class="font-medium text-blue-900">Seasonal Status</h4>
                        <div>
                            @if ($selectedIngredient->is_seasonal)
                                @if ($selectedIngredient->isInSeason())
                                    <span
                                        class="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">In
                                        Season</span>
                                @else
                                    <span
                                        class="px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">Out
                                        of Season</span>
                                @endif
                            @else
                                <span class="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full">Not
                                    Seasonal</span>
                            @endif
                        </div>
                    </div>
                ...
                @if ($selectedIngredient->is_seasonal)
                    <div class="space-y-4">
                        <div class="bg-white rounded-lg shadow p-4">
                            <h5 class="font-medium text-gray-900 mb-2">Season Information</h5>
                            <div class="space-y-2">
                                @if (!$selectedIngredient->isInSeason())
                                    <p class="text-sm text-gray-600">
                                        Next Season Starts:
                                        {{ $selectedIngredient->getNextSeasonStart()?->format('F Y') }}
                                    </p>
                                @endif
                                <p class="text-sm text-gray-600">
                                    Peak Season: {{ $selectedIngredient->isPeakSeason() ? 'Yes' : 'No' }}
                                </p>
                            </div>
                        </div>

                        <div class="bg-white rounded-lg shadow p-4">
                            <h5 class="font-medium text-gray-900 mb-2">Seasonal Price Adjustments</h5>
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Regular Price:</span>
                                    <span class="font-medium">{{ number_format($selectedIngredient->cost, 2) }}
                                        DH</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Seasonal Adjustment:</span>
                                    <span
                                        class="font-medium {{ $selectedIngredient->getSeasonalPriceAdjustment() > 0 ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $selectedIngredient->getSeasonalPriceAdjustment() > 0 ? '+' : '' }}{{ number_format($selectedIngredient->getSeasonalPriceAdjustment(), 1) }}%
                                    </span>
                                </div>
                            </div>
                        </div>
        ...
        <!-- Wastage Metrics Modal -->
        <x-modal wire:model="showWastageModal" name="showWastageModal">
            <x-slot name="title">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Wastage Metrics</h3>
            </x-slot>
            @if ($selectedIngredient)
                <div class="mb-6 bg-red-50 rounded-lg p-4">
                    <div class="flex justify-between items-center">
                        <h4 class="font-medium text-red-900">Total Wastage</h4>
                        <p class="text-lg font-bold text-red-600">
                            {{ number_format($selectedIngredient->calculateWastage(now()->subDays(30), now()), 2) }}
                            {{ $selectedIngredient->unit }}
                        </p>
                    </div>
                    <p class="mt-2 text-sm text-red-600">
                        Wastage Rate:
                        {{ number_format($selectedIngredient->calculateWastagePercentage(now()->subDays(30), now()), 1) }}%
                    </p>
                </div>

                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-white rounded-lg shadow p-4">
                        <h5 class="font-medium text-gray-900 mb-2">Turnover Rate</h5>
                        <p
                            class="text-2xl font-bold {{ $selectedIngredient->calculateTurnoverRate(now()->subDays(30), now()) > 4 ? 'text-green-600' : 'text-yellow-600' }}">
                            {{ number_format($selectedIngredient->calculateTurnoverRate(now()->subDays(30), now()), 1) }}x
                        </p>
                        <p class="text-sm text-gray-600 mt-1">Monthly turnover</p>
                    </div>

                    <div class="bg-white rounded-lg shadow p-4">
                        <h5 class="font-medium text-gray-900 mb-2">Usage Rate</h5>
                        <p class="text-2xl font-bold text-blue-600">
                            {{ number_format($selectedIngredient->calculateUsageRate(now()->subDays(30), now()), 1) }}
                        </p>
                        <p class="text-sm text-gray-600 mt-1">{{ $selectedIngredient->unit }}/day</p>
                    </div>
                </div>

                <div class="bg-white rounded-lg shadow p-4">
                    <h5 class="font-medium text-gray-900 mb-2">Stock Efficiency</h5>
                    <div class="flex items-center">
                        <div class="flex-1">
                            <div class="h-4 bg-gray-200 rounded-full">
                                @php $efficiency = $selectedIngredient->calculateStockEfficiency(now()->subDays(30), now()) @endphp
                                <div class="h-4 rounded-full {{ $efficiency >= 80 ? 'bg-green-500' : ($efficiency >= 60 ? 'bg-yellow-500' : 'bg-red-500') }}"
                                    style="width: {{ $efficiency }}%"></div>
                            </div>
                        </div>
                        <span
                            class="ml-4 text-lg font-bold {{ $efficiency >= 80 ? 'text-green-600' : ($efficiency >= 60 ? 'text-yellow-600' : 'text-red-600') }}">
