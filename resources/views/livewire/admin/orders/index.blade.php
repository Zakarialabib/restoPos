<div>
    ...
    <x-theme.breadcrumb :title="__('Order Management')" :parent="route('admin.orders')" :parentName="__('Order Management')">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($this->orders as $order)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <x-table.td>
                            ...
                            <div class="text-xs text-gray-500">
                                {{ __('Profit') }}:
                                {{-- {{ number_format($order->getProfit(), 2) }} DH --}}
                            </div>
                        </x-table.td>
                        <x-table.td>
                    ...
                    @foreach ($order->items as $item)
                        <tr class="bg-gray-50">
                            <x-table.td colspan="7" class="p-4">
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <p class="font-medium text-gray-900">{{ __('Item Details') }}</p>
                                        <p class="text-gray-500">{{ $item->product->name ?? __('Custom Juice') }}</p>
                                        <p class="text-gray-500">{{ __('Quantity') }}: {{ $item->quantity }}</p>
                                        <p class="text-gray-500">{{ __('Unit Price') }}:
                                            {{ number_format($item->price, 2) }} DH</p>
                                        <p class="text-gray-500">{{ __('Subtotal') }}:
                                            {{ number_format($item->getSubtotal(), 2) }} DH</p>
                                    </div>
                                    <div>
                                        @if ($item->details)
                                            <p class="font-medium text-gray-900">{{ __('Customization') }}</p>
                                            @if (isset($item->details['fruits']))
                                                <p class="text-gray-500">{{ __('Fruits') }}:
                                                    {{ implode(', ', $item->details['fruits']) }}</p>
                                            @endif
                                            @if (isset($item->details['base']))
                                                <p class="text-gray-500">{{ __('Base') }}:
                                                    {{ $item->details['base'] }}</p>
                                            @endif
                                            @if (isset($item->details['sugar']))
                                                <p class="text-gray-500">{{ __('Sugar') }}:
                                                    {{ $item->details['sugar'] }}</p>
                                            @endif
                                            @if (isset($item->details['size']))
                                                <p class="text-gray-500">{{ __('Size') }}:
                                                    {{ $item->details['size'] }}</p>
                                            @endif
                                            @if (isset($item->details['addons']) && count($item->details['addons']) > 0)
                                                <p class="text-gray-500">{{ __('Addons') }}:
                                                    {{ implode(', ', $item->details['addons']) }}</p>
                                            @endif
                                        @endif
                                    </div>
                                </div>
                                        ...
                                        <div class="mt-2 space-y-1">
                                            <p class="text-sm text-gray-900">
                                                ...
                                                <span @class([
                                                    ...
                                                    'bg-blue-100 text-blue-800' =>
                                                        $selectedOrder->status === \App\Enums\OrderStatus::Preparing,
                                                    'bg-green-100 text-green-800' =>
