<div>
    ...
    <div class="bg-retro-cream min-h-screen font-poppins" x-data="{ loading: false }">
        <div class="container mx-auto p-4 sm:p-6 lg:p-8" x-data="{
            step: @entangle('step'),
            selectedAddons: @entangle('selectedAddons'),
            selectedDriedFruits: @entangle('selectedDriedFruits'),
            cart: @entangle('cart'),
            ...
            <h2 class="text-4xl font-bold mb-4 text-center text-retro-blue">{{ __('Compose Your Dried Fruits') }}</h2>
            <p class="text-center mb-8 text-retro-green text-lg">
                {{ __('Follow the steps to create your perfect dried fruit blend.') }}
            </p>

            <!-- Onboarding Section -->
            @if (empty($composableDriedFruits))
                <div class="mb-12">
                    <h3 class="text-2xl font-semibold mb-4 text-retro-orange">
                        {{ __('Popular Composed Dried Fruits') }}
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        @foreach ($composableDriedFruits->take(4) as $driedFruit)
                            <div
                                class="bg-retro-yellow rounded-lg shadow-lg overflow-hidden transform transition duration-300 hover:scale-105">
                                <img src="{{ $driedFruit->image }}" alt="{{ $driedFruit->name }}"
                                    class="w-full h-48 object-cover" onerror="this.onerror=null">
                                <div class="p-4">
                                    <h4 class="text-xl font-semibold text-retro-blue mb-2">{{ $driedFruit->name }}</h4>
                                    <p class="text-retro-green mb-3">{{ $driedFruit->description }}</p>
                                    <p class="text-retro-orange font-bold text-lg">{{ $driedFruit->price }} DH</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="lg:w-1/4 order-2 lg:order-1">
                    <div class="border-retro-orange border-solid border-4 text-white rounded-lg shadow-lg p-6">
                        <h3 class="text-2xl text-retro-orange font-bold mb-4">
                            {{ __('Your Dried Fruits') }}
                        </h3>
                        <div class="flex flex-col gap-y-6 rounded-lg mb-6 text-retro-blue">
                            @if (count($selectedDriedFruits) > 0)
                                <div>
                                    <span class="font-medium">
                                        {{ __('Dried Fruits') }}:
                                    </span>
                                    <p class="text-sm mb-1">
                                        {{ implode(', ', $this->driedFruits->whereIn('id', $selectedDriedFruits)->pluck('name')->toArray()) }}
                                    </p>
                                </div>
                            @endif
                            ...
                            @endif
                            @if (count($selectedDriedFruits) === 0 && count($selectedAddons) === 0)
                                                ...
                                                <div class="w-1/2">
                                                    ...
                                                    <div class="flex flex-col text-retro-blue">
                                                        ...
                                                        <o><strong>
                                                            ...
                                                            {{ implode(', ', $ingredients['driedFruits']) }}
                                                        </o>
                                                        <o><strong>
                                                                {{ __('Add-ons:') }}
                                                            </strong>
                ...
                @else
                    ...
                    <div class="lg:w-3/4 order-1 lg:order-2 border-retro-orange border-solid border-4 rounded-lg p-4">
                        <nav class="grid grid-cols-2 md:grid-cols-4 gap-6 justify-center mb-8">
                            @foreach (['Select Dried Fruits', 'Add-ons'] as $index => $stepName)
                            ...
                            <div x-show="step === 1">
                                <div class="flex justify-between items-center my-2">
                                    ...
                                    <input type="text" wire:model.live="search"
                                        ...
                                        placeholder="Search for driedFruits...">
                                </div>
                                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                                    @foreach ($this->driedFruits as $driedFruit)
                                        <div wire:key="{{ $driedFruit->id }}">
                                            <div class="relative bg-white rounded-lg shadow-md hover:shadow-xl transition-all overflow-hidden  cursor-pointer"
                                                wire:click="toggleDriedFruit({{ $driedFruit->id }})"
                                                x-bind:class="{
                                                    'ring-4 ring-retro-orange': @js(in_array($driedFruit->id, $selectedDriedFruits))
                                                }">
                                                @if ($driedFruit->image)
                                                    <img src="{{ $driedFruit->image }}"
                                                        alt="{{ $driedFruit->name }}"
                                                ...
                                                @if (in_array($driedFruit->id, $selectedDriedFruits))
                                                    <div
